...
<%
	boolean  firstAddbuttonShow = true;
...
%>
	...
	<div id="form-container">
		...
		<%	for(QuetionPaperQuetionInformationGetModel questionInfo : questionInfoList) {
						...
						<div class="question-header">
						    <label for="question<%= question_no %>">Question <%= question_no %></label>
							 <div class="question-header-right">
								<%if(firstAddbuttonShow){%>
								    <button type="button" id="new_add_<%= question_no %>" onclick="addQuestionFormStarting(this, '<%= projectPath1 %>', '<%= question_no %>', '<%= "0".equals(optionMarks)? notOptionMarks :optionMarks %>')" class="add_button first_add_button">Add</button>
								<% firstAddbuttonShow=false;} %>
				...
				<div class="buton_div">
					<button type="button"  onclick="submitForm('<%= projectPath1 %>', '<%= question_no%>')"   class="submit-btn">Update</button>
					<button type="button" id="new_add_<%= question_no+1%>" onclick="addQuestionForm(this, '<%= projectPath1 %>', '<%= question_no+1 %>', '<%= "0".equals(optionMarks)? notOptionMarks :optionMarks %>')" class="add_button">Add</button>
					<button type="button"  onclick="deleteOneQuestion(this, '<%= projectPath1 %>', '<%= question_no%>')" class="delete-btn">Delete</button>						
				</div>
				<%}%>
		</div>
<script 	src="../../../js/admin-view-pages-js/control-questions-paper-view-pages-js/question-paper-update-question.js"></script>